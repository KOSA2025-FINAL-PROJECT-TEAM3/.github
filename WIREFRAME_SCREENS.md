...
### 🔐 인증 (Auth)
1. **로그인 페이지** - `05-login` (카카오 OAuth)
2. **추가 정보 입력 페이지** - `06-signup` (OAuth 후 개인정보 입력)
...
### 메인 플로우
```
카카오 OAuth 로그인 → 추가 정보 입력 (최초 1회) → 역할 선택 → 대시보드 (시니어/보호자)
```
...
- [x] 처방전 OCR 스캔 (UI/클라이언트)
- [x] 알약 역검색
- [x] 식단 입력 & 충돌 경고
- [x] 실시간 채팅 (Spring WebSocket/STOMP)
- [ ] 공동편집 에디터 (Hocuspocus - 선택, 현재 Front 미적용)
